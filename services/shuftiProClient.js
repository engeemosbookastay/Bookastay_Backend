...
const getAuthToken = () => {
  ...
  if (!clientId || !secretKey) {
    throw new Error(
      "SHUFTI_PRO_CLIENT_ID and SHUFTI_PRO_SECRET_KEY must be set"
    );
  }
...
const downloadAndConvertToBase64 = async (imageUrl) => {
  try {
    ...
    // Compress image using sharp - more aggressive compression
...
export const createVerificationRequest = async (guestData, retries = 3) => {
  try {
    console.log(
      "=== CREATING SHUFTI PRO VERIFICATION WITH PRE-UPLOADED ID ==="
    );
    console.log("Guest:", {
      name: guestData.name,
      email: guestData.email,
      has_id: !!guestData.id_file_url,
    });
    ...
    const reference =
      guestData.bookingReference ||
      `verify_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;

    const callbackUrl =
      process.env.SHUFTI_PRO_CALLBACK_URL ||
      "https://d2e34be85292.ngrok-free.app/api/shufti/callback";
    ...
    const documentBase64 = await downloadAndConvertToBase64(
      guestData.id_file_url
    );
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        console.log(`Sending to Shufti Pro... (Attempt ${attempt}/${retries})`);
        ...
        // If we got a response but no verification_url, don't retry
    ...
    // All retries failed
    if (lastError?.code === "ECONNRESET" || lastError?.code === "ETIMEDOUT") {
      return {
        success: false,
        error: "Connection to verification service failed after multiple attempts. Please try again later.",
        retryable: true,
      };
    }

    return lastError || { success: false, error: "Verification failed" };
